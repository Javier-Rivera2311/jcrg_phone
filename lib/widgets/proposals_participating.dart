...
class ProposalsParticipatingWidget extends StatefulWidget {
  const ProposalsParticipatingWidget({super.key});

  @override
  State<ProposalsParticipatingWidget> createState() =>
      _ProposalsParticipatingWidgetState();
}

class _ProposalsParticipatingWidgetState
    extends State<ProposalsParticipatingWidget> {
  ...
  Future<List<dynamic>> fetchProposals() async {
    ...
    final response = await http.get(
      Uri.parse(
          'https://backend-jcrgapp.onrender.com/user/proposals/participating'),
      headers: {
    ...
    if (response.statusCode == 200) {
      ...
      throw Exception(
          'Error al cargar propuestas participando (${response.statusCode})');
    }
  }
                      ...
                      children: [
                        ...
                        Text(
                          _searchQuery.isNotEmpty
                              ...
                              : 'No hay propuestas participando',
                          style: TextStyle(
                              fontSize: 18, color: Colors.grey.shade600),
                        ),
                        const SizedBox(height: 8),
                        Text(
                          'Las propuestas en estado "participando" aparecerán aquí',
                          style: TextStyle(
                              fontSize: 14, color: Colors.grey.shade500),
                        ),
                      ],
    ...
    try {
      ...
      final response = await http.post(
        ...
        body: json.encode({
          ...
          'status': status,
          'files_path':
              '/proposals/participating/${DateTime.now().millisecondsSinceEpoch}',
        }),
      );
